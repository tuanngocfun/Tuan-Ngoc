    ...
    async function UpdateStock() {
        ...
        for (let i = 0; i < data.length; i++) {
            if (document.getElementById("export-value-" + data[i].product_id).value != 0) {
                ...
                const res = await fetch('http://localhost:8080/customer/export/minhdeptrai4869@gmail.com/' + data[i].product_id + '/0355491721/' + document.getElementById("export-value-" + data[i].product_id).value,
                    {
                        headers: {
                            'Content-Type': 'application/json',
                        }
